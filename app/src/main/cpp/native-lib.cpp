...
#define JAVA_NULL 0
    ...
    void setDetPtr(JNIEnv *env, jobject thiz, DetPtr newPtr) {
        std::lock_guard<std::mutex> lock(gLock);//std::lock_guard 在构造函数中进行加锁，析构函数中进行解锁。
...
jobjectArray getRecResult(JNIEnv *env, DetPtr faceDetector, const int &size) {
    jobjectArray jDetRetArray = JNI_VisionDetRet::createJObjectArray(env, size);//创建对象集合，长度未得到的特征点size
    for (int i = 0; i < size; i++) {
        jobject jDetRet = JNI_VisionDetRet::createJObject(env);//创建对象
        env->SetObjectArrayElement(jDetRetArray, i, jDetRet);//将对象按下角标设置进去
        dlib::rectangle rect = faceDetector->getDetResultRects()[i];//获取rect

        g_pJNI_VisionDetRet->setRect(env, jDetRet, rect.left(), rect.top(),//调用成员函数
...
DLIB_FACE_JNI_METHOD(jniBitmapDet)(JNIEnv *env, jobject thiz, jobject bitmap) {
    cv::Mat rgbaMat;
    cv::Mat bgrMat;
    jniutils::ConvertBitmapToRGBAMat(env, bitmap, rgbaMat, true);//将bitmap 转换成map
    cv::cvtColor(rgbaMat, bgrMat, cv::COLOR_RGBA2BGR);//颜色空间转换
    DetPtr mDetPtr = getDetPtr(env, thiz);//指针句柄
    jint size = mDetPtr->Detect(bgrMat);//通过成员方法得到特征点size,并将特征点存储到vector中
